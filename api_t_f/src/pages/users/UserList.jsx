...
import { Users, Plus, Trash2, Edit, Loader2, X, UserCircle } from 'lucide-react';
...
export default function UserList() {
  ...
  const handleOpenEditModal = (u) => {
    ...
    const currentRole = Array.isArray(u.roles) && u.roles.length > 0 
      ? (typeof u.roles[0] === 'string' ? u.roles[0] : u.roles[0].role_name || 'Cashier')
      : 'Cashier';
  ...
  const handleSubmit = async (e) => {
    ...
    // រៀបចំ payload ដោយបន្ថែម password_confirmation
    const payload = {
    ...
    // បន្ថែម password និង password_confirmation លុះត្រាតែមានការបញ្ចូល password
  ...
  const formatRoles = (roles) => {
    ...
    return roles.map(r => (typeof r === 'string' ? r : r.role_name || r.name || 'Unknown')).join(', ');
  };
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage system staff and access roles</p>
        </div>
        <button
            ...
            {loading ? (
              <tr>
                <td colSpan="4" className="p-8 text-center text-slate-400">
                  <span className="inline-flex items-center gap-2"><Loader2 className="animate-spin" /> Loading users...</span>
                </td>
              </tr>
            ) : users.length > 0 ? (
              users.map((u) => {
                  ...
                  <tr key={userId} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="p-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                        roleString.includes('Admin') ? 'bg-amber-100 text-amber-700' :
                        roleString.includes('Manager') ? 'bg-blue-100 text-blue-700' :
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                <label className="text-xs font-bold text-slate-600 block mb-1">
                  Password {isEditing && <span className="text-slate-400 font-normal">(Leave blank to keep existing)</span>}
                </label>
                <input
                  type="password" 
                  required={!isEditing}
              ...
              <div className="flex gap-3 pt-3">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-2.5 bg-slate-100 text-slate-700 font-bold rounded-xl text-xs cursor-pointer">
                  Cancel
                </button>
                <button type="submit" disabled={submitting} className="flex-1 py-2.5 bg-amber-600 text-white font-bold rounded-xl text-xs flex justify-center items-center gap-2 cursor-pointer">
